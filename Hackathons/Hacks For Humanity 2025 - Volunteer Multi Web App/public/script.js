...
function filterLoadedOpportunities(opportunities) {
    ...
    // Create a copy of opportunities to filter
    let filteredResults = [...opportunities].filter(opp => opp !== null);
    
    // Regular filtering logic
    return applyRegularFilters(filteredResults);
}
...
function updateTrackedOpportunities(activities) {
    ...
    activitiesList.innerHTML = activities.length ? activities.map(activity => `
        <div class="tracked-opportunity" style="cursor: pointer;" onclick="viewTrackedOpportunity('${activity.opportunityId}', '${JSON.stringify(activity).replace(/'/g, "\\'")}')">
            <div class="opportunity-content">
                <h4>${activity.title}</h4>
                <p>Status: ${activity.status}</p>
                <p>Date: ${new Date(activity.date).toLocaleDateString()}</p>
                ${activity.organization?.name ? `<p>Organization: ${activity.organization.name}</p>` : ''}
                ${activity.remote_or_online !== undefined ? 
                    `<p>Type: ${activity.remote_or_online ? 'Remote/Online' : 'In-Person'}</p>` : ''}
            </div>
...
function viewTrackedOpportunity(opportunityId, fallbackData) {
    try {
        // Parse the fallbackData which comes as a string
        const parsedFallbackData = typeof fallbackData === 'string' ? 
            JSON.parse(fallbackData) : fallbackData;
        ...
        const opportunityDetails = {
            ...
            organization: parsedFallbackData.organization || {},
            audience: parsedFallbackData.audience || {},
            activities: parsedFallbackData.activities || []
        };

        // Store the complete opportunity data
        localStorage.setItem('selectedOpportunity', JSON.stringify(opportunityDetails));
        window.location.href = 'opportunity-details.html';
    } catch (error) {
