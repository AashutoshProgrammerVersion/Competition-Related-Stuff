...
// Update the specific opportunity endpoint
app.get('/api/opportunities/:id', async (req, res) => {
    try {
        // First check cached opportunities
        if (global.cachedOpportunities) {
            const opportunity = global.cachedOpportunities.find(
                opp => String(opp.id) === String(req.params.id)
            );
            if (opportunity) {
                return res.json({
                    success: true,
                    opportunity: {
                        ...opportunity,
                        url: opportunity.url || `https://www.volunteerconnector.org/opportunity/${opportunity.id}`,
                        organization: opportunity.organization ? {
                            ...opportunity.organization,
                            logo: opportunity.organization.logo?.startsWith('http') ? 
                                opportunity.organization.logo : 
                                `https:${opportunity.organization.logo}`
                        } : null
                    }
                });
            }
        ...
        try {
            ...
            if (response.data?.results?.length) {
                const opportunity = response.data.results.find(
                    opp => String(opp.id) === String(req.params.id)
                );
                
                if (opportunity) {
                    // Fix logo URL and ensure consistent format
