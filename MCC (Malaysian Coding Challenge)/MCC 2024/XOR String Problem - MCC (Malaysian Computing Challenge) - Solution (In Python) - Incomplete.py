...
for i in range(len(dictionary_of_all_substrings.keys())):
    ...
    if all(digit == '1' for digit in temporary_number):
        for j in range(2):
            temporary_number = transform_string(temporary_number)
        temporary_array = [0, beauty_of_string(temporary_number), 0]
        for j in range(number_of_times_to_transform - 2):
            temporary_array[2] = (temporary_array[0] * 2) + temporary_array[1]
            temporary_array[0] = temporary_array[1]
            temporary_array[1] = temporary_array[2]
        total_beauty += temporary_array[1] * list(dictionary_of_all_substrings.values())[i]
    elif sum(int(digit) for digit in temporary_number) == 1:
        beauty_of_first_number = beauty_of_string(transform_string(temporary_number))
        for j in range(number_of_times_to_transform - 1):
            if j % 2 == 0:
                beauty_of_first_number += (beauty_of_first_number - 1)
            else:
                beauty_of_first_number += (beauty_of_first_number + 1)
        total_beauty += beauty_of_first_number * list(dictionary_of_all_substrings.values())[i]
    else:
